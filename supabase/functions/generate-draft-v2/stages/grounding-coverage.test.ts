...
Deno.test("thanks/update never trigger; missing inputs are fail-safe", () => {
  assertEquals(
    assessGroundingCoverage({ intent: "thanks", chunkCount: 0 }).ungrounded,
    false,
  );
  assertEquals(
    assessGroundingCoverage({ intent: "update", chunkCount: 0 }).ungrounded,
    false,
  );
...
Deno.test("directive forbids invented refusals and includes the customer's ask", () => {
  const block = buildOwnsTheCaseBlock({
    customerAsk: "Kan I kontakte Maxgaming?",
    intent: "complaint",
  });
  assert(block.includes("Kan I kontakte Maxgaming?"));
  assert(/opfind ALDRIG|ALDRIG en afvisning/i.test(block));
  assert(
    /undersøger .* vender tilbage|undersøger det og vender tilbage/i.test(
      block,
    ),
  );
...
Deno.test("a near-duplicate historical ticket does not factually ground a case", () => {
  const r = assessGroundingCoverage({
    intent: "product_question",
    chunkCount: 0,
    verifiedFactsCount: 0,
    structuredFactsCount: 0,
    strongTicketExampleCount: 1,
  });
  assertEquals(r.ungrounded, true);
  assertEquals(r.reason, "no_chunks_no_facts");
});

Deno.test("a near-duplicate historical ticket does not override matcher abstention", () => {
  const r = assessGroundingCoverage({
    intent: "product_question",
    chunkCount: 3,
    matcherAbstained: true,
    verifiedFactsCount: 0,
    structuredFactsCount: 0,
    strongTicketExampleCount: 2,
  });
  assertEquals(r.ungrounded, true);
  assertEquals(r.reason, "matcher_abstained_no_facts");
});
