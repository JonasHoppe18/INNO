...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!shop_id || (!thread_id && !email_data)) {
      return new Response(
        JSON.stringify({
          error: "shop_id and either thread_id or email_data required",
        }),
        {
          status: 400,
    ...
    const result = await runDraftV2Pipeline({
      thread_id,
      message_id,
      shop_id,
      supabase,
    ...
    console.log(
      `[generate-draft-v2] thread=${thread_id} latency=${latency_ms}ms skipped=${
