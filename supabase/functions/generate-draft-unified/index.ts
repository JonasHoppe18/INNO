...
Afslut ikke med signatur – signaturen tilføjes automatisk senere.`;
    ...
    // Fire-and-forget: AI auto-tagging baseret på emne + genereret svar
    if (supabase && workspaceId && threadId && OPENAI_API_KEY) {
      (async () => {
        try {
          ...
          const tagList = workspaceTags
            .map((t: { id: string; name: string; category?: string }) =>
              `- ID: ${t.id} | Navn: ${t.name}${t.category ? ` | Kategori: ${t.category}` : ""}`
            )
            .join("\n");

          const userMessage = [
            `Emne: ${emailData.subject || "(intet emne)"}`,
            "",
            "Svar sendt til kunden:",
            finalText?.trim() || "(intet svar)",
            "",
            "Tilgængelige tags:",
            tagList,
          ].join("\n");
          ...
          const raw = tagData?.choices?.[0]?.message?.content ?? "{}";
          let parsed: { tag_ids?: unknown[]; solution_summary?: string };
          try { parsed = JSON.parse(raw); } catch { return; }

          const validIds = new Set(workspaceTags.map((t: { id: string }) => t.id));
          ...
          const summary = typeof parsed.solution_summary === "string"
            ? parsed.solution_summary.trim().slice(0, 500)
            : "";
          if (summary) {
            await supabase
              .from("mail_threads")
              .update({ solution_summary: summary })
              .eq("id", threadId);
          }
        } catch (tagErr: any) {
          console.warn("[auto-tag] fejl:", tagErr?.message);
        }
      })();
    }
