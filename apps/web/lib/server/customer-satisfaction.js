...
export function customerSatisfactionDatabaseValues(body, workspaceId, existing = {}) {
  ...
  return {
    ...
    customer_only: true,
    subject: text(body?.subject, existing.subject || CUSTOMER_SATISFACTION_DEFAULTS.subject),
    headline: text(body?.headline, existing.headline || CUSTOMER_SATISFACTION_DEFAULTS.headline),
    intro: text(body?.intro, existing.intro || CUSTOMER_SATISFACTION_DEFAULTS.intro),
    thank_you: text(body?.thankYou, existing.thank_you || CUSTOMER_SATISFACTION_DEFAULTS.thankYou),
    company_name: text(body?.company, existing.company_name || ""),
    sender_name: text(body?.senderName, existing.sender_name || ""),
    footer: text(body?.footer, existing.footer || CUSTOMER_SATISFACTION_DEFAULTS.footer),
    accent_color: accent(body?.accent, existing.accent_color || CUSTOMER_SATISFACTION_DEFAULTS.accent),
    logo_position: logoPosition(body?.logoPosition, existing.logo_position || CUSTOMER_SATISFACTION_DEFAULTS.logoPosition),
    logo_size: logoSize(body?.logoSize, existing.logo_size || CUSTOMER_SATISFACTION_DEFAULTS.logoSize),
    language_mode: languageMode(body?.languageMode, existing.language_mode || CUSTOMER_SATISFACTION_DEFAULTS.languageMode),
