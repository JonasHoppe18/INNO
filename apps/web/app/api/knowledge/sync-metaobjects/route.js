...
export async function POST(request) {
  ...
  try {
    ...
    console.info(JSON.stringify({ event: "knowledge.sync.start", provider: "shopify_metaobject", requested_shop_id: requestedShopId || null, resolved_shop_id: shop.shop_id, workspace_id: shop.workspace_id ?? null }));
    const result = await syncMetaobjects({ serviceClient, creds: shop });
    console.info(JSON.stringify({ event: "knowledge.sync.wrote", provider: "shopify_metaobject", source_provider: "shopify_metaobject", resolved_shop_id: shop.shop_id, workspace_id: shop.workspace_id ?? null, fetched_count: Number(result?.synced ?? 0), rows_written: Number(result?.updated_chunks ?? 0), rows_updated: Number(result?.updated_chunks ?? 0), rows_deleted: 0 }));
    return NextResponse.json({ success: true, platform: "shopify", ...result }, { status: 200 });
  } catch (error) {
    console.error(JSON.stringify({ event: "knowledge.sync.error", provider: "shopify_metaobject", error: error instanceof Error ? error.message : "Sync failed" }));
    return NextResponse.json(
