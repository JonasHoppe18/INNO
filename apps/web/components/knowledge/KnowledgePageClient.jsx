...
export function KnowledgePageClient() {
  ...
  const hydrateSavedReplyEditor = (value, imagesOverride = savedReplyImages) => {
    requestAnimationFrame(() => {
      const editor = savedReplyEditorRef.current;
      if (!editor) return;
      editor.innerHTML = toSavedReplyEditorPreviewHtml(value, imagesOverride);
    });
  };
  ...
  const syncSavedReplyEditorToState = () => {
    ...
    setSavedReplyContent(toSavedReplyStorageHtml(sanitized));
  };
  ...
  const insertSavedReplyInlineImage = (image) => {
    const editor = savedReplyEditorRef.current;
    ...
    if (!editor || !contentId || !mimeType || !contentBase64) return;
    editor.focus();
    document.execCommand(
      "insertHTML",
      false,
      `<img src="data:${mimeType};base64,${contentBase64}" data-content-id="${contentId}" alt="${escapeHtml(
        String(image?.filename || "Inline image")
      )}">`
    );
    syncSavedReplyEditorToState();
  };
  ...
  const openCreateSavedReplyModal = () => {
    ...
    resetSavedReplyForm();
    if (draft) {
      const draftImages = (Array.isArray(draft.images) ? draft.images : []).map((image) => ({
        ...image,
        delivery_mode: normalizeSavedReplyImageDeliveryMode(image?.delivery_mode || image?.deliveryMode),
      ...
      setSavedReplyImages(draftImages);
      setSavedReplyModalOpen(true);
      hydrateSavedReplyEditor(draft?.content || "", draftImages);
      return;
    }
    setSavedReplyModalOpen(true);
  ...
  const openEditSavedReplyModal = (reply) => {
    ...
    const baseImages = Array.isArray(reply?.images)
      ? reply.images
      : reply?.image && typeof reply.image === "object"
        ? [reply.image]
        : [];
    const nextImages = (Array.isArray(draft?.images) ? draft.images : baseImages).map((image) => ({
      ...image,
      delivery_mode: normalizeSavedReplyImageDeliveryMode(image?.delivery_mode || image?.deliveryMode),
      content_id: normalizeContentId(image?.content_id || image?.contentId, image?.filename),
    }));
    setSavedReplyImages(nextImages);
    setSavedReplyModalOpen(true);
    hydrateSavedReplyEditor(draft?.content || String(reply?.content || ""), nextImages);
  };
  ...
  useEffect(() => {
    ...
    const hasAny = Boolean(
      savedReplyTitle.trim() ||
        savedReplyCategory.trim() ||
        hasContent ||
        (Array.isArray(savedReplyImages) && savedReplyImages.length)
    );
  ...
  const handleSavedReplyImageChange = async (event) => {
    ...
    try {
      for (const file of files) {
        ...
        const filename = String(file.name || "saved-reply-image");
        const contentId = makeSavedReplyContentId(filename);
        const image = {
          filename,
          mime_type: mimeType,
        ...
        setSavedReplyImages((prev) => [...(Array.isArray(prev) ? prev : []), image].slice(0, 10));
        insertSavedReplyInlineImage(image);
      }
    } catch (_error) {
  ...
  const removeSavedReplyImageAt = (index) => {
    ...
    setSavedReplyImages((prev) => (Array.isArray(prev) ? prev.filter((_, i) => i !== index) : []));
    const contentId = normalizeContentId(image?.content_id || image?.contentId);
    if (!contentId) return;
    const editor = savedReplyEditorRef.current;
    if (!editor) return;
    const nextHtml = String(editor.innerHTML || "").replace(
      new RegExp(
  ...
  return (
    <>
      ...
      >
        <DialogContent
          className="max-h-[92vh] w-[min(960px,96vw)] overflow-y-auto"
          onInteractOutside={(event) => {
            event.preventDefault();
          }}
        >
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Upload inserts image inline by default. You can switch each image to attachment mode.
                </p>
              </div>
