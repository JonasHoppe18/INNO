    ...
    <div className="mx-auto w-full max-w-[1240px]">
      ...
      <Sheet open={sourceSheetOpen} onOpenChange={setSourceSheetOpen}>
        <SheetContent side="right" className="flex w-full flex-col gap-0 overflow-y-auto p-0 sm:max-w-xl">
          ...
          <div className="flex-1 space-y-5 px-6 py-5">
            <div className="grid gap-2"><Label htmlFor="greenfield-source-title">Source title</Label><Input id="greenfield-source-title" value={sourceForm.title} onChange={(event) => setSourceForm((current) => ({ ...current, title: event.target.value }))} placeholder="e.g. Product manual" disabled={sourceSaving} /></div>
            ...
            <div className="rounded-lg border border-indigo-100 bg-indigo-50/50 px-3 py-2.5 text-xs leading-5 text-indigo-800">Use headings (`## Reset`, `## Pairing`, …) to split the source. Candidates are always created as Draft.</div>
            <div className="grid gap-2"><Label htmlFor="greenfield-source-content">Raw source</Label><Textarea id="greenfield-source-content" value={sourceForm.content} onChange={(event) => setSourceForm((current) => ({ ...current, content: event.target.value }))} disabled={sourceSaving} placeholder={'## Reset\n\n1. Turn the device off.\n2. Hold the power button for 15 seconds.\n\n## Pairing\n\n1. ...'} className="min-h-[360px] resize-y font-mono text-xs leading-5" /></div>
          </div>
