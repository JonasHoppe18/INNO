...
test("HEIC attachments are detected by MIME type or filename", () => {
  ...
  assert.equal(isHeicAttachment({ filename: "customer-photo.jpg", mimeType: "image/jpeg" }), false);
});

test("HEIC preview filenames use a browser-compatible extension", () => {
  assert.equal(buildHeicPreviewFilename("customer-photo.heic"), "customer-photo.jpg");
  assert.equal(buildHeicPreviewFilename("customer-photo.HEIF"), "customer-photo.jpg");
});
